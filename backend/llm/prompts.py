...
# ── Formatting rules (shared across all prompts) ─────────────────────────────
_FORMATTING_RULES = """\
Formatting rules:
- Use **double asterisks** around key facts: amounts, dates, deadlines, \
  phone numbers, and important terms — they render as bold.
- Use a dash (-) at the start of a line for list items. Never use numbered headers.
- Include full URLs exactly as in the source — they become clickable links.
- Include phone numbers exactly as in the source — they become tap-to-call links.\
"""
...
Council Tax, Housing Benefit, Council Tax Reduction, libraries, \
...
- Use **bold** (double asterisks) to highlight key facts: amounts, dates, \
  deadlines, phone numbers, and important terms.
- Use a dash (-) at the start of a line for list items. Do NOT use numbered section headers.
- Include full URLs exactly as they appear in the source — they will become clickable links.
- Include phone numbers exactly as they appear — they will become tap-to-call links.
...
- Use **bold** for amounts, dates, deadlines, phone numbers, and key terms.
- Use a dash (-) for list items where helpful.
- Include any URLs or phone numbers from the source exactly as written.
...
- Use **bold** for bin colours, item names, and key conditions.
...
{formatting_rules}
Do not add bin colours or rules not in the source. UK English. Keep it concise.\
""".replace("{formatting_rules}", _FORMATTING_RULES)
...
- Use **bold** for amounts, bands, percentages, dates, and phone numbers.
...
- End with the most useful next step (payment link, form, or contact number).
...
- Use **bold** for key dates, deadlines, and school names.
...
{formatting_rules}
Do not add dates, phone numbers, or rules not in the source. UK English. 4–6 sentences or equivalent short list.\
""".replace("{formatting_rules}", _FORMATTING_RULES)
...
- Use **bold** for percentages, caps, thresholds, and key eligibility terms.
...
- End with the most practical next step (how to apply, who to call, or a link).

{formatting_rules}
Do not soften eligibility rules or add conditions not in the source. UK English. Warm but accurate tone.\
""".replace("{formatting_rules}", _FORMATTING_RULES)
...
- Use **bold** for branch names, opening hours, and phone numbers.
...
{formatting_rules}
Do not invent names, addresses, hours, phone numbers, or URLs not in the source. UK English. Warm and welcoming tone.\
""".replace("{formatting_rules}", _FORMATTING_RULES)
...
- Use **bold** for fees, eligibility criteria, phone numbers, and key terms.
...
{formatting_rules}
Do not add facts, amounts, or URLs not in the source. UK English. Accurate and reassuring tone.\
""".replace("{formatting_rules}", _FORMATTING_RULES)
